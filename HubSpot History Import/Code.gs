/*********************************************
 * HubSpot History Import - HubSpot menu actions
 *
 * Baseline copy of "INT Sheet Template/HubSpot.gs" adjusted to import historical
 * collaborations. Differences vs. the INT template:
 *   - Campaign Name and Client name are free single-line text (no dropdown
 *     dependency); enrichment still backfills Client name from "Dropdown Values"
 *     when a match happens to exist, but never requires it.
 *   - New manually-entered columns imported as-is: Amount, EXT Amount, Contract,
 *     Activation URL, Activation Type.
 *   - Activation custom-object properties are imported (Activation Name,
 *     Activation Type, Script URL, Preview URL, Publication Date, Activation URL),
 *     and the deal-level progress flags (Pitching Status, Contract Signed,
 *     Script Approved, Preview Approved, Published) are imported to the deal.
 *   - Enrichment sets: Activation Name = "[Deal name] - [Activation Type]";
 *     Pipeline = "Campaign Management Pipeline"; Deal stage = "Published";
 *     Pitching Status = "Approved"; Script Approved / Preview Approved /
 *     Published = "Yes"; Contract Signed = "Yes" only when Contract is filled.
 *     Everything else is inherited from the INT enrichment.
 *********************************************/
...
function enrichCreatorListRowsInBatches() {
  return HISTORY_HUBSPOT_MENU_.enrichCreatorListRowsInBatches();
}

function importCreatorListToHubSpotOnly() {
  return HISTORY_HUBSPOT_MENU_.importCreatorListToHubSpotOnly();
}

function downloadCreatorListWoodpeckerCsv() {
  return HISTORY_HUBSPOT_MENU_.downloadCreatorListWoodpeckerCsv();
}

function syncDropdownValuesFromHubSpot() {
  return HISTORY_HUBSPOT_MENU_.syncDropdownValuesFromHubSpot();
}

const HISTORY_HUBSPOT_MENU_ = (function () {
  ...
  // History-import HubSpot object routing.
  //
  // The shared importer maps each sheet column to a HubSpot property by matching
  // the column label against every object it loads. Some history columns are
  // ambiguous across objects, so we prefix the header with the owning object
  // name before sending it (the importer documents this exact disambiguation,
  // e.g. "Deals Record ID"). Prefixing restricts the importer's search to that
  // single object, which is what makes routing deterministic.
  //
  //   * "Activation Type" exists on BOTH deals and the Activation object.
  //   * "Script Approved" / "Preview Approved" / "Published" are deal flags, but
  //     the Activation object also tracks script/preview/publication, so the
  //     label could otherwise match two objects and fail with an "ambiguous
  //     column" error.
  //
  // "Activation Name", "Activation Type" and "Activation URL" already begin with
  // the object alias "Activation", so they self-route to the Activation object
  // and do not need an entry here.
  const HUBSPOT_IMPORT_OBJECT_PREFIX_BY_COLUMN_ = {
  ...
  function importCreatorListToHubSpotOnly() {
    ...
    const startRow1 = context.headerRow1 + 1;
  ...
  function enrichCreatorRow_(row, header, clientNameByCampaignName) {
    ...
    // Pipeline: historical collaborations live in the Campaign Management Pipeline
    set("Pipeline", "Campaign Management Pipeline");

    // Deal stage: historical collaborations are already Published
    set("Deal stage", "Published");

    // Historical collaborations are already completed, so mark the deal-level
    // progress flags. Pitching Status is a HubSpot dropdown with no "Yes" option;
    // "Approved" is its terminal positive value. Script Approved / Preview
    // Approved / Published are Yes/No booleans (the "Yes" label imports as true).
    set("Pitching Status", "Approved");
    set("Script Approved", "Yes");
    set("Preview Approved", "Yes");
    set("Published", "Yes");

    // Contract Signed is "Yes" only when a Contract (URL) was provided; otherwise
  ...
  function applyCreatorCampaignNames_(row, header, creatorLabel, campaignName, changeMap) {
    ...
    if (label && campaign) {
      ...
      // Fall back to a manually-entered Deal name so Activation Name can still be
      // derived when the creator label / campaign can't be reconstructed.
      dealName = String(getValueByHeader_(row, header, "Deal name") || "").trim();
    }

    if (dealName) {
      // Activation Name = "[Deal name] - [Activation Type]". Activation Type is a
      // manually-entered column; when it is blank, fall back to the Deal name
      // alone rather than leaving a trailing " - ".
  ...
  function buildHubSpotImportPayload_(ss, header, rowItems, excludedColIndexes, importLabel) {
    ...
    const activeHeaders = activeColIndexes.map(i => resolveHubSpotImportHeaderName_(header[i]));
  ...
  function shouldExcludeCreatorListColumnFromHubSpotImport_(headerName) {
    ...
    if (!text) return true;
    return HUBSPOT_IMPORT_EXCLUDED_COLS_.has(text);
  }

  // Returns the header to send to the shared importer, prefixed with the owning
  // HubSpot object name when the column needs deterministic routing (see
  // HUBSPOT_IMPORT_OBJECT_PREFIX_BY_COLUMN_). The match is case-insensitive so it
  // tolerates header capitalization differences.
  function resolveHubSpotImportHeaderName_(headerName) {
    const text = String(headerName || "").trim();
    if (!text) return text;

    const normalized = normalizeHeaderName_(text);
    const columnNames = Object.keys(HUBSPOT_IMPORT_OBJECT_PREFIX_BY_COLUMN_);
    for (let i = 0; i < columnNames.length; i++) {
      if (normalizeHeaderName_(columnNames[i]) === normalized) {
        return HUBSPOT_IMPORT_OBJECT_PREFIX_BY_COLUMN_[columnNames[i]] + " " + text;
      }
    }
