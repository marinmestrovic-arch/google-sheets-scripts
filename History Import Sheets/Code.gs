...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu("🧰 Scripts")
    .addItem("Enrichment", "openEnrichSubsheetPicker")
    .addItem("Import to HubSpot", "openHubSpotImportSubsheetPicker")
    .addItem("Woodpecker Export", "openWoodpeckerExportSubsheetPicker")
    .addToUi();
...
// Menu entry: only opens the subsheet picker dialog. Enrichment itself starts
// when the user picks a subsheet and clicks Run in the dialog.
function openEnrichSubsheetPicker() {
  return INT_HUBSPOT_MENU_.openEnrichSubsheetPicker();
}

// Menu entry: only opens the subsheet picker dialog. The import itself starts
// when the user picks a subsheet and clicks Run in the dialog.
function openHubSpotImportSubsheetPicker() {
  return INT_HUBSPOT_MENU_.openHubSpotImportSubsheetPicker();
}

// Menu entry: only opens the subsheet picker dialog. The export itself starts
// when the user picks a subsheet and clicks Run in the dialog.
function openWoodpeckerExportSubsheetPicker() {
  return INT_HUBSPOT_MENU_.openWoodpeckerExportSubsheetPicker();
}

// Called from the subsheet picker dialog via google.script.run.
function runWoodpeckerExportForSubsheet(sheetName) {
  return INT_HUBSPOT_MENU_.runWoodpeckerExportForSubsheet(sheetName);
}

// Called from the subsheet picker dialog via google.script.run.
function runEnrichmentForSubsheet(sheetName) {
  return INT_HUBSPOT_MENU_.runEnrichmentForSubsheet(sheetName);
}

// Polled by the progress dialog via google.script.run.
function getScriptActionProgress() {
  return INT_HUBSPOT_MENU_.getScriptActionProgress();
}

// Called from the subsheet picker dialog via google.script.run.
function runHubSpotImportForSubsheet(sheetName) {
  return INT_HUBSPOT_MENU_.runHubSpotImportForSubsheet(sheetName);
}
...
const INT_HUBSPOT_MENU_ = (function () {
  ...
  // All API keys live in Script Properties (Project Settings → Script
  // properties), never in code: HUBSPOT_API_KEY, OPENAI_API_KEY, YOUTUBE_API_KEY.
  ...
  const HUBSPOT_DROPDOWN_VALUES_SYNC_CONFIG_ = {
    ...
    optionColumns: [
      ...
      {
        columnName: HUBSPOT_DROPDOWN_VALUES_COLUMNS_.industry,
        objectConfigKey: "client",
  ...
  function openEnrichSubsheetPicker() {
    showSubsheetPickerDialog_("Which subsheet do I enrich?", "runEnrichmentForSubsheet", {
      withProgress: true,
      actionLabel: "enrichment"
    });
  }

  // Runs the full enrichment for the chosen subsheet. Returns a summary string
  // shown in the progress dialog; progress along the way is published via
  // setScriptActionProgress_ and polled by the dialog.
  function runEnrichmentForSubsheet(sheetName) {
    const sheet = resolveSubsheetByName_(sheetName);
    if (!sheet) throw new Error(`Subsheet "${sheetName}" not found.`);

    try {
      return runEnrichmentForSheet_(sheet, String(sheetName));
    } catch (e) {
      setScriptActionProgress_(`❌ Enrichment of "${sheetName}" failed: ${e.message}`, 0, "error");
      throw e;
    }
  }

  function runEnrichmentForSheet_(sheet, sheetName) {
    ...
    if (!context) {
      const message = `"${sheetName}" has no data.`;
    ...
    while (nextStartRow1 <= stopRow1) {
      ...
      const percent = Math.min(99, Math.round(((nextStartRow1 - startRow1) / scanTotal) * 100));
      setScriptActionProgress_(
        `Enriching "${sheetName}" — rows ${rowItems[0].row1}–${rowItems[rowItems.length - 1].row1} ` +
        `of ${stopRow1} (${totalRows} row(s) done so far)…`,
        percent,
        "running"
      );
  ...
  function openHubSpotImportSubsheetPicker() {
    showSubsheetPickerDialog_("Which subsheet do I import to HubSpot?", "runHubSpotImportForSubsheet", {
      withProgress: true,
      actionLabel: "HubSpot import"
    });
  }

  // Runs the full HubSpot import for the chosen subsheet. Returns a summary
  // string shown in the progress dialog (which then auto-closes); errors are
  // thrown so the dialog displays them and stays open.
  function runHubSpotImportForSubsheet(sheetName) {
    const sheet = resolveSubsheetByName_(sheetName);
    if (!sheet) throw new Error(`Subsheet "${sheetName}" not found.`);

    try {
      return runHubSpotImportForSheet_(sheet, String(sheetName));
    } catch (e) {
      setScriptActionProgress_(`❌ HubSpot import of "${sheetName}" failed: ${e.message}`, 0, "error");
      throw e;
    }
  }

  function finishActionWithInfo_(message) {
    setScriptActionProgress_(message, 100, "done");
    showSpreadsheetToast_(message);
    Logger.log(`ℹ️ ${message}`);
    return message;
  ...
  // Detects what the chosen subsheet imports (deals / client campaigns /
  // clients) from its columns and dispatches to the matching import flow.
  function runHubSpotImportForSheet_(sheet, sheetName) {
    const ss = sheet.getParent();
    setScriptActionProgress_(`Reading "${sheetName}"…`, 0, "running");

    const data = sheet.getDataRange().getValues();
    if (!data || data.length < 2) return finishActionWithInfo_(`"${sheetName}" has no data.`);

    const detected = detectHubSpotImportKind_(data);
    if (!detected) {
      throw new Error(
  ...
  function runDealImportForSheet_(ss, sheet, sheetName, importSpec) {
    ...
    if (missingColumns.length > 0) {
      throw new Error(
        `"${sheetName}" is missing required column(s): ` +
        missingColumns.join(", ")
      );
    }
  ...
  // object, rows deduped by the primary column, all headers routed to the
  // object via the alias prefix.
  function runSimpleObjectImportForSheet_(ss, sheetName, detected, data) {
    const spec = detected.spec;
    const header = detected.header;
    const primaryCol = findHeaderIndex_(header, spec.primaryColumn);
    if (primaryCol === -1) {
      throw new Error(`"${sheetName}" is missing required column: ${spec.primaryColumn}`);
    }
    ...
    for (let r = detected.headerRow0 + 1; r < data.length; r++) {
      const row = data[r];
      if (isBlankRow_(row)) continue;

      const primary = String(row[primaryCol] || "").trim();
      if (!primary) {
        issues.push(`Row ${r + 1}, column "${spec.primaryColumn}": is blank.`);
        continue;
      }

      const primaryKey = primary.toLowerCase();
      if (seenPrimary[primaryKey]) {
        skippedDuplicates++;
        continue;
      }
    ...
    if (rowItems.length === 0) {
      return finishActionWithInfo_(`No importable rows found in "${sheetName}".`);
    }

    setScriptActionProgress_(
      `Preparing ${rowItems.length} ${spec.label} row(s) from "${sheetName}"…`,
      10,
      "running"
    );

    const payload = buildSimpleHubSpotImportPayload_(ss, header, rowItems, spec, sheetName);
    if (!payload) {
      throw new Error(`No importable columns with values found in "${sheetName}".`);
    }

    const payloads = splitHubSpotImportPayloadByRows_(payload);
    const payloadValidationError = validateHubSpotImportPayloads_(payloads);
    if (payloadValidationError) {
      throw new Error("Validation failed:\n" + payloadValidationError);
    }

    const result = sendHubSpotImportPayloads_(payloads, null, sheetName);
    const message = buildSimpleImportSuccessMessage_(spec, result, skippedDuplicates);
    setScriptActionProgress_(message, 100, "done");
    showSpreadsheetToast_(message);
    Logger.log(`✅ ${message}`);
    return message;
  ...
  function openWoodpeckerExportSubsheetPicker() {
    showSubsheetPickerDialog_("Which subsheet do I export for Woodpecker?", "runWoodpeckerExportForSubsheet", {
      withProgress: true,
      actionLabel: "Woodpecker export"
    });
  }

  // Runs the Woodpecker CSV export for the chosen subsheet. Returns either an
  // info string, or { message, download } — the progress dialog triggers the
  // browser download from `download` and then auto-closes.
  function runWoodpeckerExportForSubsheet(sheetName) {
    const sheet = resolveSubsheetByName_(sheetName);
    if (!sheet) throw new Error(`Subsheet "${sheetName}" not found.`);

    try {
      return runWoodpeckerExportForSheet_(sheet, String(sheetName));
    } catch (e) {
      setScriptActionProgress_(`❌ Woodpecker export of "${sheetName}" failed: ${e.message}`, 0, "error");
      throw e;
    }
  }

  function runWoodpeckerExportForSheet_(sheet, sheetName) {
    const ss = sheet.getParent();
    setScriptActionProgress_(`Reading "${sheetName}"…`, 0, "running");

    const finishWithInfo = function (message) {
      setScriptActionProgress_(message, 100, "done");
      showSpreadsheetToast_(message);
      Logger.log(`ℹ️ ${message}`);
      return message;
    ...
    const context = getCreatorListSheetContext_(sheet);
    if (!context) return finishWithInfo(`"${sheetName}" has no data.`);
    ...
    setScriptActionProgress_(`Building Woodpecker CSV from "${sheetName}"…`, 50, "running");
    const exportRows = buildCreatorListWoodpeckerExportRows_(context.data, header, context.headerRow1);
    if (exportRows.length === 0) {
      return finishWithInfo(`No creator rows found to export in "${sheetName}".`);
    }

    const filename = buildWoodpeckerExportFilename_(ss.getName() + " - " + sheetName);
    const csvBytes = toCsvBytes_([WOODPECKER_EXPORT_HEADERS_].concat(exportRows));
    const message = `Woodpecker export of "${sheetName}" complete. ${exportRows.length} row(s) exported.`;
    setScriptActionProgress_(message, 100, "done");
    showSpreadsheetToast_(message);
    Logger.log(`✅ ${message}`);
    ...
    return {
      ...
      download: {
        filename: filename,
        base64: Utilities.base64Encode(csvBytes),
        mimeType: "text/csv;charset=utf-8"
      }
    };
  }

  // Protected template sheets: people duplicate these to create their working
  // copies, so the templates themselves are never a target of any menu action.
  const SUBSHEET_PROMPT_EXCLUDED_NAMES_ = new Set([
    "Dropdown Values",
    "Deal Import",
    "Campaign Import",
    "Client Import"
  ...
  // What a subsheet imports is detected from its columns, so copies work no
  // matter what they are renamed to. signatureColumns are distinctive per kind
  // (no overlap between kinds). aliasPrefix is prepended to every header sent
  // to the shared importer so each column routes to the right HubSpot object
  // (bare headers like "Client name" or "Country/Region" would otherwise be
  // routed by the importer's generic hints to the wrong object).
  const HUBSPOT_IMPORT_KIND_SPECS_ = [
    {
      key: "deal",
      label: "Deals",
      importFlow: "deal",
    ...
    },
    {
      ...
      aliasPrefix: "Client Campaign",
      primaryColumn: "Campaign name",
      // "Client name" is the one column that is NOT a Client Campaign
      // property: it routes to the Client object so HubSpot's multi-object
      // import associates the campaign with that Client record (same row =
      // automatic association, same as "Client name" on Deal Import sheets).
      columnAliasOverrides: { "Client name": "Client" },
      signatureColumns: [
        "Agency Fee",
        "INT Sheet",
        "EXT Sheet",
  ...
  function detectHubSpotImportKind_(data) {
    let best = null;

    HUBSPOT_IMPORT_KIND_SPECS_.forEach(function (spec) {
      const headerRow0 = detectHeaderRow0ForColumns_(data, spec.signatureColumns, 10);
      if (headerRow0 === -1) return;

      const header = (data[headerRow0] || []).map(v => String(v || "").trim());
      let score = 0;
      spec.signatureColumns.forEach(function (columnName) {
        if (findHeaderIndex_(header, columnName) !== -1) score++;
      });

      if (score >= HUBSPOT_IMPORT_KIND_MIN_SIGNATURE_SCORE_ && (!best || score > best.score)) {
        best = { spec: spec, score: score, headerRow0: headerRow0, header: header };
      }
    });
  ...
  // Ask the user which subsheet to run a menu action on, via a modal dialog
  // with a dropdown of the available subsheets. Picking one and clicking Run
  // invokes the given top-level function with the chosen sheet name.
  //
  // With options.withProgress the dialog stays open while the server function
  // runs: it turns into a progress banner (message + bar, polled from
  // getScriptActionProgress every 2s) and, being modal, blocks edits to the
  // spreadsheet until the run finishes and the user closes it.
  function showSubsheetPickerDialog_(question, serverFnName, options) {
    const withProgress = !!(options && options.withProgress);
    const ui = SpreadsheetApp.getUi();
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const candidateNames = ss.getSheets()
      .map(function (sheet) { return sheet.getName(); })
      .filter(function (name) { return !SUBSHEET_PROMPT_EXCLUDED_NAMES_.has(name); });
    if (candidateNames.length === 0) {
      ui.alert("❌ No subsheets available in this spreadsheet.");
      return;
    }

    const activeName = ss.getActiveSheet().getName();
    const defaultName = candidateNames.indexOf(activeName) !== -1 ? activeName : candidateNames[0];
    const optionsHtml = candidateNames.map(function (name) {
      const escaped = escapeHtml_(name);
      return '<option value="' + escaped + '"' +
        (name === defaultName ? " selected" : "") +
        '>' + escaped + '</option>';
    }).join("");

    const fireAndCloseScript =
      'document.getElementById("run").onclick=function(){' +
      'var name=document.getElementById("subsheet").value;' +
      'google.script.run.' + serverFnName + '(name);' +
      'google.script.host.close();' +
      '};';

    const actionLabel = String((options && options.actionLabel) || "the script");
    const progressScript =
      'var pollTimer=null;' +
      'var ACTION_LABEL=' + JSON.stringify(actionLabel) + ';' +
      'function setMsg(t){document.getElementById("progress-message").textContent=t;}' +
      'function setPct(p){document.getElementById("progress-bar").style.width=Math.max(0,Math.min(100,p))+"%";}' +
      'function finish(msg,ok){' +
      'if(pollTimer){clearInterval(pollTimer);pollTimer=null;}' +
      'setMsg(msg);' +
      'document.getElementById("progress-note").style.display="none";' +
      'if(ok){setPct(100);setTimeout(function(){google.script.host.close();},2000);}' +
      'else{document.getElementById("close").style.display="inline-block";}' +
      '}' +
      'function poll(){' +
      'google.script.run.withSuccessHandler(function(p){' +
      'if(!p||!pollTimer)return;' +
      'if(p.message)setMsg(p.message);' +
      'if(typeof p.percent==="number")setPct(p.percent);' +
      '}).getScriptActionProgress();' +
      '}' +
      'function triggerDownload(d){' +
      'try{' +
      'var binary=atob(d.base64||"");' +
      'var bytes=new Uint8Array(binary.length);' +
      'for(var i=0;i<binary.length;i++){bytes[i]=binary.charCodeAt(i);}' +
      'var blob=new Blob([bytes],{type:d.mimeType||"text/csv;charset=utf-8"});' +
      ...
      'setMsg("Starting "+ACTION_LABEL+" of \\""+name+"\\"\\u2026");' +
      'pollTimer=setInterval(poll,2000);' +
      'google.script.run' +
      '.withSuccessHandler(function(res){' +
      'if(res&&typeof res==="object"){' +
      'if(res.download)triggerDownload(res.download);' +
      'finish(res.message||"Done.",true);' +
      '}else{finish(res||"Done.",true);}' +
      '})' +
      '.withFailureHandler(function(err){finish("\\u274C "+(err&&err.message?err.message:err),false);})' +
      '.' + serverFnName + '(name);' +
      '};';
    ...
    const html = HtmlService.createHtmlOutput(
      ...
      '<body style="font-family:Arial,sans-serif;padding:16px;margin:0;">' +
      '<div id="picker">' +
      '<label for="subsheet" style="display:block;margin:0 0 8px;font-size:13px;">Subsheet:</label>' +
      '<select id="subsheet" style="width:100%;padding:6px;font-size:14px;">' + optionsHtml + '</select>' +
      '<div style="margin-top:16px;text-align:right;">' +
      '<button onclick="google.script.host.close()" style="margin-right:8px;padding:6px 14px;">Cancel</button>' +
      '<button id="run" style="padding:6px 14px;">Run</button>' +
      '</div>' +
      '</div>' +
      '<div id="progress" style="display:none;">' +
      '<div id="progress-message" style="font-size:13px;min-height:36px;white-space:pre-wrap;">Starting…</div>' +
      '<div style="margin-top:10px;background:#eee;border-radius:4px;height:10px;overflow:hidden;">' +
      '<div id="progress-bar" style="background:#1a73e8;height:100%;width:0%;transition:width .4s;"></div>' +
      '</div>' +
      '<div id="progress-note" style="margin-top:10px;font-size:11px;color:#666;">' +
      'Running. Please keep this window open and do not edit the sheet until it finishes.' +
      '</div>' +
      '<div style="margin-top:14px;text-align:right;">' +
      '<button id="close" style="display:none;padding:6px 14px;" onclick="google.script.host.close()">Close</button>' +
      '</div>' +
      '</div>' +
      '<script>' + (withProgress ? progressScript : fireAndCloseScript) + '</script>' +
      '</body></html>'
    )
      .setWidth(360)
      .setHeight(withProgress ? 210 : 150);

    ui.showModalDialog(html, String(question || "Which subsheet?"));
  }

  const SCRIPT_ACTION_PROGRESS_CACHE_KEY_ = "SUBSHEET_ACTION_PROGRESS_V1";

  // Progress is best-effort: published to the script cache so the dialog can
  // poll it; a cache hiccup must never break the action itself.
  function setScriptActionProgress_(message, percent, state) {
    try {
      CacheService.getScriptCache().put(
        SCRIPT_ACTION_PROGRESS_CACHE_KEY_,
        JSON.stringify({
      ...
      Logger.log("⚠️ Could not publish progress: " + e);
    }
  }

  function getScriptActionProgress() {
    try {
      const raw = CacheService.getScriptCache().get(SCRIPT_ACTION_PROGRESS_CACHE_KEY_);
  ...
  function resolveSubsheetByName_(sheetName) {
    const name = String(sheetName || "").trim();
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = name ? ss.getSheetByName(name) : null;
    if (!sheet) {
      showSpreadsheetToast_(`❌ Subsheet "${name}" not found.`);
      Logger.log(`❌ Subsheet "${name}" not found.`);
      return null;
    }
    return sheet;
  }
  ...
  function enrichProfileFieldsViaLlm_(sheet, rowItems, header, dropdownValuesByHeader) {
    ...
    if (!apiKey) {
      Logger.log("ℹ️ OpenAI API key not set (script property OPENAI_API_KEY). AI enrichment will be skipped, but explicit email and phone extraction will still run.");
    }
  ...
  function importCreatorListToHubSpot_(ss, sheet, header, rowItems, importSummary, sheetName, importSpec) {
    ...
    setScriptActionProgress_(`Preparing ${rowItems.length} row(s) from "${sheetName}"…`, 10, "running");
  ...
  // the shared web app, reporting per-batch progress along the way.
  function sendHubSpotImportPayloads_(payloads, saveMarkers, sheetName) {
    const reportPayloadProgress = function (index, total) {
      setScriptActionProgress_(
        `Importing "${sheetName}" to HubSpot — batch ${index + 1} of ${total}…`,
        Math.min(99, 20 + Math.round((index / Math.max(total, 1)) * 75)),
        "running"
      );
    };

    if (hasHubSpotSharedImporterLibrary_()) {
      return runHubSpotSharedLibraryImports_(payloads, saveMarkers, reportPayloadProgress);
    }

    const webAppUrl = String(HUBSPOT_SHARED_IMPORT_WEB_APP_URL_ || "").trim();
    if (webAppUrl) {
      return runHubSpotSharedWebAppImports_(payloads, webAppUrl, saveMarkers, reportPayloadProgress);
    }
  ...
  // Payload for single-object imports (Client Campaigns / Clients). Headers
  ...
  function buildSimpleHubSpotImportPayload_(ss, header, rowItems, spec, sheetName) {
    ...
    const aliasOverrideByKey = {};
    Object.keys(spec.columnAliasOverrides || {}).forEach(function (columnName) {
      aliasOverrideByKey[normalizeHeaderName_(columnName)] = spec.columnAliasOverrides[columnName];
    });

    const activeHeaders = activeColIndexes.map(function (i) {
      const overrideAlias = aliasOverrideByKey[normalizeHeaderName_(header[i])];
      const alias = overrideAlias !== undefined ? overrideAlias : spec.aliasPrefix;
    ...
    return {
      ...
      importLabel: spec.label
    };
  }

  function buildSimpleImportSuccessMessage_(spec, result, skippedDuplicates) {
    const results = Array.isArray(result.importResults) ? result.importResults : [];
    const rowCount = results.reduce((sum, r) => sum + Number(r.rowCount || 0), 0);
    const allDone = results.length > 0 && results.every(r => String(r.state || "").trim() === "DONE");
    const importLines = results.map(r => {
      const importState = String(r.state || "STARTED").trim() || "STARTED";
      return "- Rows " + Number(r.rowCount || 0) +
    ...
    return (
      ...
      spec.label + " rows imported: " + rowCount + "\n" +
      "Imports:\n" + importLines.join("\n") + "\n" +
      "Skipped duplicate " + spec.primaryColumn + " rows: " + Number(skippedDuplicates || 0) + "\n\n" +
      (allDone
        ? "HubSpot finished the import."
  ...
  // The shared importer derives the HubSpot CSV filename from these names, and
  // HubSpot's import API rejects the upload when the filename declared in the
  // request body doesn't byte-match the uploaded multipart filename — which
  // happens with non-ASCII characters (e.g. "Meštrović"). Strip diacritics and
  // any remaining non-ASCII so the names survive the round trip.
  function toAsciiSafeHubSpotImportName_(value, fallback) {
    let text = String(value || "");
    try {
      text = text.normalize("NFKD");
    } catch (e) {
      // normalize unavailable — fall through to the raw string.
    }
  ...
  function runHubSpotSharedLibraryImports_(payloads, onPayloadImported, onPayloadStarted) {
    const importResults = [];
    const dealRecordIds = [];
    ...
    payloads.forEach((payload, index) => {
      ...
      if (typeof onPayloadStarted === "function") onPayloadStarted(index, payloads.length);
  ...
  function runHubSpotSharedWebAppImports_(payloads, webAppUrl, onPayloadImported, onPayloadStarted) {
    const importResults = [];
    const dealRecordIds = [];
    let savedRecordIds = 0;
    let savedTimestamps = 0;
  ...
  function markApiKeyInvalid_() {
    ...
    if (!YT_API_KEY_INVALID_LOGGED_) {
      Logger.log("❌ YouTube API key invalid. Update the YOUTUBE_API_KEY script property and rerun.");
  ...
  return {
    openEnrichSubsheetPicker: openEnrichSubsheetPicker,
    openHubSpotImportSubsheetPicker: openHubSpotImportSubsheetPicker,
    runEnrichmentForSubsheet: runEnrichmentForSubsheet,
    runHubSpotImportForSubsheet: runHubSpotImportForSubsheet,
    getScriptActionProgress: getScriptActionProgress,
    openWoodpeckerExportSubsheetPicker: openWoodpeckerExportSubsheetPicker,
