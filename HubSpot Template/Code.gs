...
function exportToCsv() {
  ...
  if (!sh) throw new Error(`Sheet not found: "${CRM_SHEET}"`);
  ...
  // Use DISPLAY values to mimic File > Download > CSV behavior
  ...
  if (emailIdx === -1) throw new Error(`Header not found: "${EMAIL_HEADER}"`);

  // Only rows with Email filled
  const exportRows = rows.filter(r => {
    const email = r[emailIdx];
    return email !== "" && email !== null && String(email).trim() !== "";
  });

  if (exportRows.length === 0) {
    SpreadsheetApp.getUi().alert("No rows with a filled Email column to export.");
    return;
  }
  ...
  showDownloadDialog_([
    {
      name: `Export - ${baseName}.csv`,
      bytes: toCsvBytes_([headers, ...exportRows])
    }
  ]);
}
...
function showDownloadDialog_(files) {
  const fileData = files.map(f => ({
    name: f.name,
    // base64 of BYTES (safe for unicode)
    base64: Utilities.base64Encode(f.bytes)
  }));
    ...
    <html>
      <body>
        <script>
          ...
          files.forEach(f => downloadFile(f.name, f.base64));
  ...
  SpreadsheetApp.getUi().showModalDialog(html, "Exporting CSV...");
}
